...
To create natural-looking {% include tooltip.html tooltip="GraspConfiguration" text="grasp configurations" %} during {% include tooltip.html tooltip="GraspSynthesisMethod" text="grasp synthesis" %}, we need to [bake the object](object_baking.0.14.0.html#object-baking).
...
[grasp studio](unity_component_vggraspstudio.0.14.0.html#grasp-studio) can be used to add grasps into database through DG. 
...
| Jump Primary Grasp | when user triggers grasp, object jumps to the grasped position in the hand, using the labeled primary grasp(s) in the grasp DB | using primary grasp(s) is needed particularly in situations when an object should be grasped in some particular ways (e.g. how to grasp scissors). **Note**: to use this interaction type, you should have added some primary grasps in to grasp DB through [grasp studio](unity_component_vggraspstudio.0.14.0.html#grasp-studio).| 
...
| Jump Primary Grasp | SG | &#x2611; Needed to us SG since {% include tooltip.html tooltip="PrimaryGrasp" text="primary grasp" %} is grasp(s) in the DB which is only used during SG synthesis.| 

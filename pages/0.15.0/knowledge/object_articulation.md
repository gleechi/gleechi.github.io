...
permalink: object_articulation.0.15.0.html
folder: mydoc
---
...
{% include tip.html content="You can use [GetObjectJointState](virtualgrasp_unityapi.0.15.0.html#getobjectjointstate) and [GetObjectSecondaryJointState](virtualgrasp_unityapi.0.15.0.html#getobjectsecondaryjointstate) to get current joint state and secondary joint state (in the case of PLANAR joint) respectively." %}
...
And this is only relevant for [push without physics](push_interaction.0.15.0.html#push-without-physics) setup for push interaction.
...
but rather specifying a preferred hand approach direction which is only used for [pushable object selection](push_interaction.0.15.0.html#from-object-selection-to-push-without-physics). How object moves is defined by anchor together with other joint parameters." %}
...
To learn more details of how to setup pushable object see [push interaction](push_interaction.0.15.0.html).
...
| Index Pushable | {% include tooltip.html tooltip="InteractionAffordance" text="interaction affordance" %}: Can be pushed by the index finger; only relevant when setup [push interaction without physics](push_interaction.0.15.0.html#push-without-physics).| for all joint types | 
|-------|--------|---------|
...
The image below shows Unity's [VG_Articulation](unity_component_vgarticulation.0.15.0.html) component used to setup all parameters of object articulation.

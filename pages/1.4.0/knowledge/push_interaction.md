...
permalink: push_interaction.1.4.0.html
folder: knowledge
toc: true
---
...
VirtualGrasp not only provides solutions to [grasp interaction](grasp_interaction.1.4.0.html), 
...
VirtualGrasp provides a solution to make push interaction setup easier and intuitive through [object articulation](object_articulation.1.4.0.html), without relying on physical simulations.

{% include callout.html content="Most important concepts relevant to push interaction are explained in [object articulation](object_articulation.1.4.0.html) page." %}
...
{% include callout.html content="Note that a pushable object is selected before the finger tip touches the object, and the selection can be visualized by using [VG_HintVisualizer](unity_component_vghintvisualizer.1.4.0.html)." %}
...
Some complaints have arrised from VG users about the finger penetrating through pushing surface of an object. The foundamental reason is on the mechanism of [how object is pushed](#from-object-selection-to-push-without-physics) for VG's non-physical pushing setup. To explain more explicitly, the object is not pushed by force in physical setup but rather moved because it is "dragged" by the position of the sensor controlled finger tip, therefore the finger tip could go through the pushing surface. In next version we will introduce a proxy hand which will be forced to stay on the pushing surface. 
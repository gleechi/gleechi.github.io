...
version: 0.9.4
permalink: object_articulation.0.9.4.html
folder: mydoc
---
...
And this is only relevant for [push without physics](push_interaction.0.9.4.html#push-without-physics) setup for push interaction.
...
but rather specifying a preferred hand approach direction which is only used for [pushable object selection](push_interaction.0.9.4.html#from-object-selection-to-push-without-physics). How object moves is defined by pivot together with other joint parameters." %}
...
To learn more details of how to setup pushable object see [push interaction](push_interaction.0.9.4.html).
...
| Index Pushable | {% include tooltip.html tooltip="InteractionAffordance" text="interaction affordance" %}: Can be pushed by the index finger; only relevant when setup [push interaction without physics](push_interaction.0.9.4.html#push-without-physics).| for all joint types | 
|-------|--------|---------|
...
The image below shows Unity's [VG_Articulation](unity_component_vgarticulation.0.9.4.html) component used to setup all parameters of object articulation.

...
permalink: unity_component_myvirtualgrasp.0.9.6.html
folder: mydoc
---
...
Select an [VG_AutoSetup](virtualgrasp_unityapi.0.9.6.html#vg_autosetup) option from the dropdown menu, then click "Setup" to automatically adjust the [Sensor options](#sensor) and [SensorSetting:FingerControlType](#finger-control-type)). 
...
| Remote | enable this if you want to use this avatar to reflect networked data (i.e. listening to another client over network in a multiplayer scenario), as explained in [Multiplayer Interaction](multiplayer_interaction.0.9.6.html), or the [VG_Networking Component](unity_component_vgnetworing.0.9.6.html).| 
...
<!--Check the **Replay** option if you want to use this avatar not for runtime-control, but for replaying recorded sensor data, as explained in [Sensor Record and Replay](sensor_record_replay.0.9.6.html), or the [VG_Recorder Component](unity_component_vgrecorder.0.9.6.html).-->
...
| Finger Control Type | supported | specify how sensor controls the finger motion. See [Finger Control Type](virtualgrasp_unityapi.0.9.6.html#vg_fingercontroltype). | 
...
You can set the default [grasp Interaction](grasp_interaction.0.9.6.html#grasp-interaction) parameters for all objects in the scene globally in Grasp Interaction Settings.
See detailed explanation of the parameters in page [grasp Interaction](grasp_interaction.0.9.6.html#grasp-interaction).

{% include callout.html content= "Note that Synthesis Method and Interaction Type can be set locally for each object by attaching [VG_Interactable](unity_component_vginteractable.0.9.6.html#unity-component-vginteractable) component to the object. These local settings will overwrite the global settings for that object." %} 
...
|-------|--------|
| INTERNAL_SELECTION | VG inherent graspable object selection method as described in [grasp interaction](grasp_interaction.0.9.6.html#from-object-selection-to-grasp-synthesis) | 
...
| Parameters | Description |
|-------|--------|
| Save Debug Files | Enabling this and running the application will create a vg_tmp subdirectory in your project and save sources that are used for different purposes (see [debug files](debug_files.0.9.6.html)). | 
| Physics Default Contact Offset | Overwrite Unity physics contact offset for more accurate collision detection. Currently only relevant for the experimental feature [push with physics](push_interaction.0.9.6.html#push-with-physics).| 

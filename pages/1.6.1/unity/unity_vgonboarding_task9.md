...
permalink: unity_vgonboarding_task9.1.6.1.html
folder: mydoc
---
...
The combined mesh should be marked as an {% include tooltip.html tooltip="VGInteractable" text="interactable" %} object by adding [VG_Articulation](unity_component_vgarticulation.1.6.1.html) to its transform. Then bake the object using [VG_BakingClient](unity_component_vgbakingclient.1.6.1.html). 
Once the object is baked, [VG_GraspEditor](unity_component_vggraspeditor.1.6.1.html) is used to add primary grasps on both left and right hands. 
...
For each hand side, a [VG_FingerAnimator](unity_component_vgfingeranimator.1.6.1.html) and a [VG_AnimationDriver](unity_component_vganimationdriver.1.6.1.html) component is added to the object. See the images below showing the setup for the left hand: 
...
The previous step of animating fingers allows the non-thumb fingers to close to simulate the squeezing motion on the articulated plier part. Now the plier part, SM_Piler2, that is in contact with these non-thumb fingers should be animated in sync with the finger animation. To do that [VG_ObjectAnimator](unity_component_vgobjectanimator.1.6.1.html) is used:

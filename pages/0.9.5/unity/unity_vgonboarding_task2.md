...
````
is the unity scene showing how [VG_Articulation](unity_component_vgarticulation.0.9.5.html) component is used to setup antenna and knobs' articulation settings.
...
[SetObjectSelectionWeight](virtualgrasp_unityapi.0.9.5.html#setobjectselectionweight) to tune up and down the selection weight in runtime.
...
````
is the script to use API function [GetSensorPose](virtualgrasp_unityapi.0.9.5.html#getsensorpose) to check when sensor controlled wrist position (**sensor_pos**) is too far from the avatar hand's wrist position (**hand.m_hand.position**) by a certain threshold (**m_disassembleDistance**), then API function [ChangeObjectJoint](virtualgrasp_unityapi.0.9.5.html#changeobjectjoint) is used to change object {% include tooltip.html tooltip="JointType" text="joint type" %} to freely floating.

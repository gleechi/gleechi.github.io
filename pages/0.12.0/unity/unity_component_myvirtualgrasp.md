...
permalink: unity_component_myvirtualgrasp.0.12.0.html
folder: mydoc
---
...
Select an [VG_AutoSetup](virtualgrasp_unityapi.0.12.0.html#vg_autosetup) option from the dropdown menu, then click "Setup" to automatically adjust the [Sensor options](#sensor) and [SensorSetting:FingerControlType](#finger-control-type)). 
...
| Replay | enable this if you want to use this avatar for replay recorded sensor data, as explained in [Sensor Record and Replay](sensor_record_replay.0.12.0.html), or the [VG_Recorder Component](unity_component_vgrecorder.0.12.0.html). This is a feature that is not available in the free version. |
...
<!--enable this if you want to use this avatar to reflect networked data (i.e. listening to another client over network in a multiplayer scenario), as explained in [Multiplayer Interaction](multiplayer_interaction.0.12.0.html), or the [VG_Networking Component](unity_component_vgnetworing.0.12.0.html).-->
<!--Check the **Replay** option if you want to use this avatar not for runtime-control, but for replaying recorded sensor data, as explained in [Sensor Record and Replay](sensor_record_replay.0.12.0.html), or the [VG_Recorder Component](unity_component_vgrecorder.0.12.0.html).-->
...
| Finger Control Type | supported | specify how sensor controls the finger motion. See [Finger Control Type](virtualgrasp_unityapi.0.12.0.html#vg_fingercontroltype). | 
...
After you have setup how your avatar's hands are controlled, you can use the Sensor Settings interface to specify [Trigger Button](virtualgrasp_unityapi.0.12.0.html#vg_vrbutton) globally for all of the sensors.
...
[VG_Articulation](unity_component_vgarticulation.0.12.0.html) component is a default entry, but this method also allows you to quickly adjust your project if you already have a layer or a component that marks your {% include tooltip.html tooltip="VGInteractable" text="interactable" %} objects.

Once an object is marked as {% include tooltip.html tooltip="VGInteractable" text="interactable" %}, it will be supported by VG's [grasp](grasp_interaction.0.12.0.html) and [push](push_interaction.0.12.0.html) interactions. 
-->
...
You can set the default [grasp Interaction](grasp_interaction.0.12.0.html#grasp-interaction) parameters for all objects in the scene globally in Global Grasp Interaction Settings.
See detailed explanation of the parameters in page [grasp Interaction](grasp_interaction.0.12.0.html#grasp-interaction).

{% include callout.html content= "Note that Synthesis Method, Interaction Type, Throw Velocity Scale and Throw Angular Velocity Scale can be set locally for each object by attaching [VG_Interactable](unity_component_vginteractable.0.12.0.html#unity-component-vginteractable) component to the object. These local settings will overwrite the global settings for that object." %} 
...
|-------|--------|
| INTERNAL_SELECTION | VG inherent graspable object selection method as described in [grasp interaction](grasp_interaction.0.12.0.html#from-object-selection-to-grasp-synthesis) | 
...
## Debug Settings
{% include image.html file="unity/unity_vg_debug_settings_0_12_0.png" alt="VG Debug Settings." caption="VG debug settings." %}

| Parameters | Description |
|-------|--------|
| Export Scene in Runtime | Enabling this and running the application will create a vg_tmp subdirectory in your project and save sources that are used for different purposes. (see [create debug files](debug_files.0.12.0.html#creating-debug-files)) | 
| Export Scene in Editor | Alternatively to check "Export Scene in Runtime", pressing Export Scene in Edit will simulate a launch of the VG plugin from the Unity Editor, thus without the need of launching the scene. This option is provided for convenience, but objects that are not in your scene yet will not be included. (see [create debug files](debug_files.0.12.0.html#creating-debug-files))|
| Contact Offset | Overwrite Unity physics contact offset for more accurate collision detection. (This is an obsolete feature and will be removed in next version!)| 
